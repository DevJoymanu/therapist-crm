...
from notes.models import ScoreEntry, SessionNote, ScoringCriterion
...
class ScoringServiceTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.therapist = User.objects.create_user(username="therapist", password="pass12345")
        self.other_therapist = User.objects.create_user(username="other", password="pass12345")
        self.patient = Patient.objects.create(
            therapist=self.therapist, first_name="Maya", last_name="Singh",
        )
        Patient.objects.create(
            therapist=self.other_therapist, first_name="Hidden", last_name="Patient",
        )
        self.session = SessionNote.objects.create(
            patient=self.patient, session_date=timezone.localdate(),
        )
    ...
    def test_lower_is_better_scores_are_inverted_for_wellness(self):
        criterion = ScoringCriterion.objects.create(
            therapist=self.therapist, name="Risk", min_score=0, max_score=10,
            alert_threshold=7, direction=ScoringCriterion.Direction.LOWER_IS_BETTER, weight=2,
        )
        score = ScoreEntry.objects.create(session=self.session, criterion=criterion, value=8)
    ...
    def test_composite_algorithm_uses_weights_and_classifies_risk(self):
        mood = ScoringCriterion.objects.create(
            therapist=self.therapist, name="Mood", min_score=0, max_score=10,
            alert_threshold=3, direction=ScoringCriterion.Direction.HIGHER_IS_BETTER, weight=1,
        )
        risk = ScoringCriterion.objects.create(
            therapist=self.therapist, name="Risk", min_score=0, max_score=10,
            alert_threshold=7, direction=ScoringCriterion.Direction.LOWER_IS_BETTER, weight=3,
        )
